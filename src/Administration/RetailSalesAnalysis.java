...
public class RetailSalesAnalysis extends JFrame {
...
private JButton button;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RetailSalesAnalysis frame = new RetailSalesAnalysis(trackID,roleid, empID, userName);
	 ...
	 * @param empID2 
	 */
	public RetailSalesAnalysis(final int trackID, int roleid, final int empID, String userName) {
		setTitle("Retail Sales & Gross Profit Analysis");
...
if(getTotSales.isSelected() == true)
{
	...
	if(getTotSales.isSelected() == false)
		...
		String[] salesColumns = new String[] { "No", "RetCode", "TransID",
				"Description", "R.Price", "Qty", "Total", "Date", "Time" };
...
if(getTotalGP.isSelected() == true)
{
						...
						toTimeS) == true)
				{
					...
					format.setGroupingUsed(true);
								...
								float total = ri.getTotalGProfit(frmDate, toDate,
										frmTime, toTime);
	...
	void insertSales(DefaultTableModel model, Date frmDate, Date toDate,
		...
		RetailInventory s = new RetailInventory();
...
noItems =sales.length;
		...
		for (int r = 0; r < noItems; r++) {
			...
			model.insertRow(r, new Object[] { r + 1, sales[r][0], sales[r][1],
					sales[r][2], format.format(Float.parseFloat(sales[r][3])), sales[r][4], format.format(Float.parseFloat(sales[r][5])),
