 ...
 private JButton button;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ViewCustomer frame = new ViewCustomer(trackid,roleid, empID, userName);
	 ...
	 * @param empID3 
	 */
	public ViewCustomer(final int trackID,int roleid, final int empID2,  String userName) {
...
if (size == 0) {
		...
		phoneT.setForeground(Color.BLUE);
		...
		nicT = new JTextField();
		nicT.setEditable(false);
		nicT.setForeground(Color.BLUE);
		nicT.setColumns(10);
		...
		if (!(txtid.length() > 0))
			...
			if (!(ii.isInteger(txtid))) {
				...
				if (empID != emp.checkCusID(empID)) {
					...
					String info[] = emp.getCusInfo(empID);
